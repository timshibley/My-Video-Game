...
int main() {
    ...
    for (int i = 0; i <5; i++){
        ...
        int xGrid = rprand_get_value(0, mainRoom.tiles[0].size() - 1);
        int yGrid = rprand_get_value(0, mainRoom.tiles.size() - 1);
        ...
        mainRoom.tiles[yGrid][xGrid]->location.entity = entityManager->findAtUltimateXY(xGrid, yGrid);
    }
    ...
    mainRoom.tiles[6][0]-> location.entity = entityManager->findAtUltimateXY(6, 0);
    setupPlayer();
    ...
    while (!WindowShouldClose()) {
        ...
        checkCollisions();
        changeFrames();
        //Simply done for debugging purposes.
